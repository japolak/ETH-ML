...
dataX = copy(X)
dataY = copy(Y)
valid_size = 0 # 0 means no validation set (ratio!)
...
#%% Cross Validation
# The following are optimal values but it takes too long to run so rather use values in comments for similar resutls
n_lambdas=31  #16
n_shuffles=100 #100
n_folds = 100 #29
lambdas=np.linspace(.35,.5,num=n_lambdas)  
...
for alpha in lambdas: #Go over all possible lambdas
    ...
    i=0; j=0
    for train_index, test_index in kfold.split(X,Y): #Cross Validation for evaluation of lambda
...
#%% Validation Set 
if valid_size > 0 :
    model=linear_model.Lasso(alpha=opt_lambda,fit_intercept=False, normalize=True)
    model.fit(X,Y)
