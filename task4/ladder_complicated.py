...
#%%
layer_sizes = [nfeatures, 500, 250, 125, 125, 125, 10]

L = len(layer_sizes) - 1  # number of layers

num_examples = x_train.shape[0]+x_unlabeled.shape[0]
num_epochs = 10
num_labeled = x_train.shape[0]

starter_learning_rate = 0.02

decay_after = 3  # epoch after which to begin learning rate decay
...
print("Initial Accuracy: ", sess.run(accuracy, feed_dict={inputs: x_valid, outputs: y_valid_mat, training: False}), "%")
...
#    images, labels = mnist.train.next_batch(batch_size)
    if batch_count_lab+batch_size > x_train.shape[0]:
        images_lab = x_train[batch_count_lab:x_train.shape[0]]
        labels = y_train[batch_count_lab:y_train.shape[0]]
        batch_count_lab = 0
    else:
        images_lab= x_train[batch_count_lab:(batch_count_lab+batch_size)]
        labels = y_train[batch_count_lab:(batch_count_lab+batch_size)]
    if batch_count_unlab+batch_size > x_unlabeled.shape[0]:
        images_unlab = x_unlabeled[batch_count_unlab:x_unlabeled.shape[0]]
        batch_count_unlab = 0
    else:
        images_unlab = x_unlabeled[batch_count_unlab:(batch_count_unlab+batch_size)]
    images = np.vstack((images_lab,images_unlab))
    labels_mat = to_categorical(labels, num_classes=nclasses) 
    
    
    sess.run(train_step, feed_dict={inputs: images, outputs: labels_mat, training: True})
    if (i > 1) and ((i+1) % (num_iter/num_epochs) == 0):
        ...
        with open('train_log', 'a') as train_log:
            ...
            log_i = [epoch_n] + sess.run([accuracy], feed_dict={inputs: x_valid, outputs: y_valid_mat, training: False})
            train_log_w.writerow(log_i)
#%%
print("Final Accuracy: ", sess.run(accuracy, feed_dict={inputs: x_valid, outputs: y_valid_mat, training: False}), "%")
