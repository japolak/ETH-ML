...
for index, row in data.iterrows():
    ...
    hbf = Stempf.shape[0]
    heartbeats = np.zeros((13, 180))
    for i,q in enumerate([.1,.25,.35,.45,.5,1,.55,.65,.75,.9,]):
        if q == 1:
            heartbeats[i, :] = np.mean(Stempf,axis=0)
        else: 
            heartbeats[i,:] = np.quantile(Stempf,q,axis=0)
    if hbf < 6:
        heartbeats[10]=Stempf[0]
        heartbeats[11]=Stempf[1]
        heartbeats[12]=Stempf[2]
    else:
        rn = np.random.randint(2,hbf-2)
        heartbeats[10]=Stempf[rn-1]
        heartbeats[11]=Stempf[rn]
        heartbeats[12] = Stempf[rn+1]
...
enco = 30 # 32 floats -> compression of factor 24.5, assuming the input is 784 floats
inp = 2340

X1n = X1 + np.random.normal(0, 3, size=X1.shape)
X2n = X2 + np.random.normal(0, 3, size=X2.shape)

input_img = Input(shape=(inp,))
encoded = Dense(180, activation='relu',kernel_initializer='random_uniform')(input_img)
encoded = Dense(60, activation='relu',kernel_initializer='random_uniform')(encoded)
encoded = Dense(30, activation='relu',kernel_initializer='random_uniform')(encoded)

encoded = Dense(enco, activation='relu',kernel_initializer='random_uniform')(encoded)

decoded = Dense(30, activation='relu',kernel_initializer='random_uniform')(encoded)
decoded = Dense(60, activation='relu',kernel_initializer='random_uniform')(decoded)
decoded = Dense(180, activation='relu',kernel_initializer='random_uniform')(decoded)
decoded = Dense(inp, activation='linear',kernel_initializer='random_uniform')(decoded)
...
autoencoder.compile(optimizer='adadelta', loss='huber_loss')

###
autoencoder.fit(X1n, X1,
                epochs=50,
                batch_size=128,
                sample_weight=sw_obs,
                shuffle=True,
                validation_data=(X2n, X2),
                verbose=2)

X1_ae = encoder.predict(X1)
X2_ae = encoder.predict(X2)
