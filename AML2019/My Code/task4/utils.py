...
# Import Data
def importData(name, path=os.getcwd(),header=0):
    pathData = path + "/" + name + ".csv"
    data = pd.read_csv(pathData,header=header)
    print(name, "csv imported!")
    if 'id' in data.columns: data = data.drop(['id'], axis=1)
    if 'Id' in data.columns: data = data.drop(['Id'], axis=1)
    return data

def importPickle(name, path=os.getcwd()):
    pathData = path + "/" + name + ".pkl"
    data = pd.read_pickle(pathData)
    print(name, "pkl imported!")
    if 'id' in data.columns: data = data.drop(['id'], axis=1)
...
# Save Data
def saveData(data,name, path=os.getcwd()):
    pathData = path + "/" + name + ".csv"
    data.to_csv(pathData)
    print("Data", name, "csv saved!")

def savePickle(data, name, path=os.getcwd()):
    pathData = path + "/" + name + ".pkl"
    data.to_pickle(pathData)
    print("Data", name, "pkl saved!")
...
def getScoreCV(clf, X1, y1, skf):
    ...
    for i, (Xf1_i, Xf2_i) in enumerate(skf.split(X1, y1)):
        ...
        clf.fit(Xf1, yf1)        
        score_skf[i] = accuracy_score(yf2, clf.predict(Xf2))
